...
{
    public class Past : System.Web.UI.Page
    {
        ...
        private JArray GetCachedEventsResponse(DateTime endDateBefore)
        {
            ...
            var key = $"past_events_{endDateBefore:yyyy-MM-dd}";
            ...
            events = GetPastEvents(endDateBefore);
            if (events != null)
            {
        ...
        private JArray GetPastEvents(DateTime endDateBefore)
        {
            ...
            var oDataFilterPast = $"$filter=Enabled ne false and EndDateTime lt datetime'{dateFilter}'"
                // + "&select=StartDateTime,Category,CalendarType,Title,Url,Thumbnail,Presenter,EventShortDescription,HasVideo&"
                + "&$orderby=StartDateTime desc&$top=100";

            ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12;

            // Logic app calls SharePoint API to fetch Events list: /sites/events/_api/web/lists/getByTitle('Events')/items
            // All odata filter, ordering, etc. should be passed via odataFilter header
            var logicAppUrl = ConfigurationManager.AppSettings["SharePointEventsLogicAppUrl"];
                
            var request = (HttpWebRequest)WebRequest.Create(logicAppUrl);
            request.Method = "GET";
            request.Timeout = 1000 * 30;
            request.Accept = "application/json";
            request.Headers.Add("odataFilter", oDataFilterPast);
